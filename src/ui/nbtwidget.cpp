...
void NbtWidget::on_load_btn_clicked() {
    ...
    if (palette.empty()) {
        QMessageBox::information(
            nullptr,
            "WARN",
            "Unable to parse NBT data (please make sure it is Bedrock Edition format)",
            QMessageBox::Yes,
            QMessageBox::Yes
        );
        return;
...
std::vector<bl::palette::compound_tag*> NbtWidget::getPaletteCopy() {
    ...
    for (int i = 0; i < ui->list_widget->count(); ++i) {
        res.push_back(dynamic_cast<bl::palette::compound_tag*>(
            dynamic_cast<NBTListItem*>(ui->list_widget->item(i))->root_->copy()
        ));
    }
    return res;
}
