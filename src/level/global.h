...
class village_data {
public:
    void reset(const std::unordered_map<std::string, std::array<bl::palette::compound_tag*, 4>>& data);
    void append_village(const bl::village_key& key, const std::string& value);

    inline std::unordered_map<std::string, std::array<bl::palette::compound_tag*, 4>>& data() { return this->data_; }
...
public:
    ...
    inline std::unordered_map<std::string, bl::palette::compound_tag*>&       data() { return this->data_; };
    inline const std::unordered_map<std::string, bl::palette::compound_tag*>& data() const { return this->data_; };
    ~general_kv_nbts();
